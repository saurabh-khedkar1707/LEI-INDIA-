...
// GET /api/technical-support-content/:id
...
export async function PUT(
  ...
  try {
    ...
    const existing = await pgPool.query(
      `SELECT id FROM "TechnicalSupportContent" WHERE section = $1 AND id != $2 LIMIT 1`,
    ...
    const result = await pgPool.query(
      `
      UPDATE "TechnicalSupportContent"
      SET
        section = $1,
        title = $2,
        content = $3,
    ...
    log.error('Failed to update technical support content', error)
    return NextResponse.json(
      { error: 'Failed to update technical support content' },
      { status: 500 },
    )
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const result = await pgPool.query(
      `DELETE FROM "TechnicalSupportContent" WHERE id = $1 RETURNING id`,
      [params.id],
    )
    ...
    log.error('Failed to delete technical support content', error)
    return NextResponse.json(
      { error: 'Failed to delete technical support content' },
      { status: 500 },
    )
  }
}
