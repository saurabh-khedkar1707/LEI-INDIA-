...
export async function GET(
  ...
  try {
    ...
    log.error('Failed to fetch returns content', error)
    return NextResponse.json(
      { error: 'Failed to fetch returns content' },
      { status: 500 },
    )
  }
}
...
export async function PUT(
  ...
  try {
    ...
    const parsed = returnsContentSchema.parse(body)

    // Check if section already exists for another record
    const existing = await pgPool.query(
      `SELECT id FROM "ReturnsContent" WHERE section = $1 AND id != $2 LIMIT 1`,
    ...
    log.error('Failed to update returns content', error)
    return NextResponse.json(
      { error: 'Failed to update returns content' },
      { status: 500 },
    )
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const result = await pgPool.query(
      `DELETE FROM "ReturnsContent" WHERE id = $1 RETURNING id`,
      [params.id],
    )
    ...
    log.error('Failed to delete returns content', error)
    return NextResponse.json(
      { error: 'Failed to delete returns content' },
      { status: 500 },
    )
  }
}
