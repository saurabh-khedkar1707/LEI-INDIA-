...
// GET /api/authorised-distributors - public (active only) or all for admin
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    log.error('Failed to fetch authorised distributors', error)
    return NextResponse.json(
      { 
        error: 'Failed to fetch authorised distributors',
...
export const POST = requireAdmin(async (req: NextRequest) => {
  ...
  try {
    ...
    const result = await pgPool.query(
      `
      INSERT INTO "AuthorisedDistributor" (
        "companyName", logo, email, phone, address, website,
        "displayOrder", active, "createdAt", "updatedAt"
      )
    ...
    log.error('Failed to create authorised distributor', error)
    return NextResponse.json(
      { error: 'Failed to create authorised distributor' },
      { status: 500 },
    )
  }
})
