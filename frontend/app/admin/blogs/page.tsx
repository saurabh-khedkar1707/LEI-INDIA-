...
export default function AdminBlogsPage() {
  ...
  const onSubmit = async (data: BlogFormData) => {
    ...
    try {
      ...
      const url = editingBlog
        ? `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/blogs/${editingBlog._id}`
        : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/blogs`
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/blogs/${id}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
              ...
              <div className="mt-2 space-y-2">
                ...
                {blogImage && (
                  <div className="relative w-full h-48">
                    <img
                      src={`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}${blogImage}`}
                      alt="Blog"
