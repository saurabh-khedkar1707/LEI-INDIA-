...
export default function AdminProductsPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/products?limit=10000`,
        {
          headers: token ? {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(null)
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/products/${productToDelete}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
